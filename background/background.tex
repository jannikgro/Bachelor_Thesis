...
Wir nehmen an, dass alle Belohnungen (engl. Reward) aus einer (endlichen) Teilmenge $\Lambda \subset \mathbb{R}$ stammen.
Dann ist für einen Zustand $s\in \mathcal{S}$ und eine Aktion $a \in \mathcal{A}$ die Wahrscheinlichkeitsdichte, in einen Zustand $s' \in \mathcal{S}$ überzugehen und dabei die Belohnung $r \in \Lambda$ zu erhalten, als $p_{s,a}(s', r)$ gegeben.
