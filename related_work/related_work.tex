...
Eine alternative Herangehensweise zu den wertebasierten Verfahren, zu denen die Q-Learning-Familie aus Abschnitt \ref{section:qlearning} gehören, sind die Policy-Gradient-Verfahren.
Während wertebasierte Verfahren zunächst Aktions- oder Zustandswerte schätzen, und daraus ihre Policy herleiten, berechnen Policy-Gradient-Verfahren direkt die Policyfunktion $\pi_\phi$ z.B. mit einem neuronalen Netz.
...
Dazu gibt es ein bedeutendes theoretisches Resultat, das eine Möglichkeit aufzeigt, den Gradienten aus gesampelten Episoden leicht zu schätzen.
Das sogenannte Policy-Gradient-Theorem lässt sich für den REINFORCE-Algorithmus auf die folgende Form bringen:
\begin{equation}
    \nabla_\phi V_{\pi_\phi}(S_0) \propto \mathbb{E}_{S_t, A_t}\left[G_t \log{\pi_\phi(S_t, A_t)}\right]
\end{equation}
